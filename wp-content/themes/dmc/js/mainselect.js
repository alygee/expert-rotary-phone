...
_.prev('ul').children('li:eq('+sel_index+')').css('background', '#caccb6').css('color', '#fff').addClass('active-li');
...
_.prev('ul').children('li').bind('click.myEventli', function() {
    ...
    // ***
    var val = $(this).parent('ul').next('select').children('option:contains('+data_sel_text+')').attr('value'); 
   ...
   _.prev('ul').children('li').each(function() {
    ...
    _.prev('.select_li').find('li').removeClass('active-li');
    $(this).css('background', '#caccb6').css('color', '#fff').addClass('active-li');
    _.prev('ul').children('li').css('cursor', 'default');
